...
//#define BMI088_accel_write_muli_reg( reg,  data, len) { bsp_select_bmi088_acc(); BMI088_write_muli_reg(reg, data, len); bsp_deselect_bmi088_acc(); }
...
//#define BMI088_gyro_write_muli_reg( reg,  data, len) { bsp_select_bmi088_gyro(); BMI088_write_muli_reg( ( reg ), ( data ), ( len ) ); bsp_deselect_bmi088_gyro(); }
...
void bsp_bmi088_clk_init(uint16_t arr, uint16_t pcs)
{
    ...
    __HAL_TIM_SET_PRESCALER(&htim8, pcs - 1);
}
...
FlagStatus bsp_spi_send(uint8_t *pData, uint32_t len, uint32_t timeout_us)
{
...
FlagStatus bsp_spi_receive(uint8_t *pData, uint32_t len, uint32_t timeout_us)
{
    bsp_bmi088_start_clk();
    for(int i = 0;i < len; ++i)
    {
		...
		for (uint32_t j = 0; j < 168; j++)
		{}
		
        // SPIx->DR = 0x55;
        hspi1.Instance->DR = 0x55;
