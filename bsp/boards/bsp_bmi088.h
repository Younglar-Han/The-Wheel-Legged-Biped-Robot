...
extern void bsp_bmi088_clk_init(uint16_t arr, uint16_t pcs);
extern uint32_t bsp_get_bmi088_clk(void);
extern FlagStatus bsp_spi_check_timeout(uint32_t timeout_us);
extern FlagStatus bsp_spi_send(uint8_t *pData, uint32_t len, uint32_t timeout_us);
extern FlagStatus bsp_spi_receive(uint8_t *pData, uint32_t len, uint32_t timeout_us);
